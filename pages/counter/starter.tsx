...
const Starter = () => {
    ...
    const handleInitializeCounter = async () => {
        ...
        const instruction = await counterAppProgram.methods
            .initialize()
            .accounts({
                payer: publicKey,
    ...
    const handleIncrementCounter = async () => {
        ...
        const instruction = await counterAppProgram.methods
            .increment()
            .accounts({
                counter: new web3.PublicKey(counterKey)
            })
            .instruction()
    ...
    useEffect(() => {
        const getInfo = async() => {
            if(connection && publicKey && counterKey){
                ...
                const counterUpdate = await counterAppProgram.account.counter.fetch(new web3.PublicKey(counterKey))

                setCount(counterUpdate.count)
            }
        }

        getInfo()
    },[counterKey, connection, publicKey, transactionSignature])
